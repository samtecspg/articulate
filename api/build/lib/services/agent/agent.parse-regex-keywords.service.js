...
module.exports = async function ({
  ...
  const keywords = await globalService.loadAllLinked({
  ...
  regexs.forEach(regex => {
    if (regex.pattern) {
      ...
      if (regexToTest.test(text)) {
        ...
        const resultToSend = Object.assign(regex, {
          resolvedRegex: resultParsed[0],
          start: startIndex,
          end: endIndex,
          regexType: 'sysRegex'
        });
        regexKeywords.push(_lodash.default.cloneDeep(resultToSend));
      }
    }
          ...
          if (match = regexToTest.exec(text)) {
            while (match) {
              if (foundRegex.indexOf(match) < 0) {
                ...
                const resultToSend = Object.assign(regex, {
                  resolvedRegex: match[0],
                  keywordValue,
                  start: startIndex,
                  end: endIndex,
                  regexType: 'keywordRegex'
                });
                regexKeywords.push(_lodash.default.cloneDeep(resultToSend));
...
//# sourceMappingURL=agent.parse-regex-keywords.service.js.map