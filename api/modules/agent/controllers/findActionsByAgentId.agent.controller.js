    ...
    Async.waterfall([
        ...
        (domains, cb) => {
            ...
            Async.map(domains, (domain, callback) => {
                ...
                const url = `/domain/${domain[1]}/action${filter !== '' ? `?filter=${filter}` : ''}`;
                server.inject(url, (res) => {
                    ...
                    return callback(null, res.result.actions);
                });
        ...
        result = _.flatten(result).map((action) => {

            return Cast(action, 'action');
        });
        result = _.sortBy(result, 'actionName');
        if (limit !== -1){
            result = result.slice(start, limit);
        }
        return reply({ actions: result, total });
    });
};
