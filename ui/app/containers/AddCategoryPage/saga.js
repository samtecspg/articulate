...
import { LOAD_PREBUILT_CATEGORIES, IMPORT_CATEGORY } from '../App/constants';
import {
...
export function* postPrebuiltCategory(payload) {
  ...
  const { data } = prebuiltCategories[category];
  try {
    yield call(
      api.post,
      toAPIPath([ROUTE_AGENT, agent.id, ROUTE_CATEGORY, ROUTE_IMPORT]),
      data,
    );
