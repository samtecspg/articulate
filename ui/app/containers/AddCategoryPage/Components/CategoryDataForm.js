...
import { Grid, Modal, Typography, Button, CircularProgress } from '@material-ui/core';
...
class CategoryDataForm extends React.Component {
  ...
  fillEmptyCards(){
    const numberOfPrebuiltCategories = Object.keys(this.props.prebuiltCategories).length;
  ...
  render(){
    ...
    return (
      <Grid className={classes.formContainer} container item xs={12}>
        <Grid className={classes.formSubContainer} id='formContainer' container item xs={12} spacing={16} justify={"space-between"}>
          <CategoryCard
            key={`newCategory`}
            category={{
              uiColor: '#00bd6f',
          ...
          />
          {Object.keys(this.props.prebuiltCategories).map((prebuiltCategory, index) => {
            return (
              <CategoryCard
        ...
        >
          <Grid container className={classes.modal}>
            ...
            <Grid item xs={12} container className={classes.modalHeader}>
              <Grid className={classes.modalCategoryNameContainer} item xs={12}>
                <Typography className={classes.modalCategoryName}>
                  {this.state.selectedCategory ? this.props.prebuiltCategories[this.state.selectedCategory].name : null}
                </Typography>
              </Grid>
              <Grid item xs={12}>
                <Typography className={classes.modalCategoryDescription}>
                  {this.state.selectedCategory ? this.props.prebuiltCategories[this.state.selectedCategory].description : null}
                </Typography>
              </Grid>
            </Grid>
            <Grid item xs={12} container className={classes.modalContent}>
              {
                ...
                this.props.prebuiltCategories[this.state.selectedCategory].samples.map((sample, index) => {

                  return (
                    <Typography className={classes.modalContentSample} variant='body1' key={`sample_${index}`}>
                      {`"${sample}"`}
                    </Typography>
                  )
                }) :
            ...
            <Grid item xs={12} container className={classes.modalFooter}>
              <Button disabled={this.props.loading} onClick={() => { this.props.importCategory(this.state.selectedCategory)}} className={classes.modalImportButton} variant='contained'>
