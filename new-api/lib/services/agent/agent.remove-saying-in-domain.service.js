...
module.exports = async function ({ id, domainId, sayingId }) {

    const { globalService, sayingService } = await this.server.services();

    try {
        const modelPath = [MODEL_AGENT, MODEL_DOMAIN, MODEL_SAYING];
        ...
        return await sayingService.remove({ SayingModel, AgentModel, DomainModel });
    }
    catch (error) {
