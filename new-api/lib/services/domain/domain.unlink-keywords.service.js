...
//const logger = require('../../../util/logger')({ name: `service:domain:unlink-keywords` });
...
module.exports = async function ({ model, keywordModels = [] }) {
    ...
    try {
        const linkPromises = keywordModels.map(async (KeywordModel) => {
            ...
            // The sayings related to this keyword are not used in the same domain
            if (sayingBelongsTo.length === 0 || _.findIndex(sayingBelongsTo) === 0) {
                await KeywordModel.unlink(model, MODEL_DOMAIN);
